...
def setup_otp():
    ...
    provisioning_url = totp.provisioning_uri(
        name="데스크톱 전원 제어", issuer_name="Desktop Power Controller")
    ...
    if power_state == 'off':
        try:
            ...
            if result.returncode == 0:
                ...
                return jsonify({
                    "status": "success",
                    "message": "컴퓨터 종료 명령을 보냈습니다."
                })
            else:
                return jsonify({
                    "status": "error",
                    "message": f"종료 오류: {result.stderr}"
                })
        except Exception as e:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return jsonify({
                    ...
                    "success",
                    "message":
                    "전원 켜기 명령을 보냈습니다. 부팅이 완료되기까지 잠시 기다려주세요."
                })
            else:
                return jsonify({
                    "status": "error",
                    "message": f"전원 켜기 오류: {result.stderr}"
                })
        except Exception as e:
            return jsonify({"status": "error", "message": f"에러 발생: {str(e)}"})
